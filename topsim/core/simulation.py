...
class Simulation:
    """
    The Simulation class wraps all actors and manages the simulation lifecycle.

    It initialises the :py:class:`~topsim.core.cluster.Cluster`,
    :py:class:`~topsim.core.buffer.Buffer`,
    :py:class:`~topsim.core.planner.Planner`,
    :py:class:`~topsim.core.scheduler.Scheduler`, and user-defined
    :py:class:`~topsim.core.instrument.Instrument` instances, then starts
    ...
    config : str or Path
        Path to the simulation JSON configuration file (compatible with
        configurations generated by ``skaworkflows``).
    instrument : type
        A user-defined subclass of :py:class:`~topsim.core.instrument.Instrument`
        (e.g. :py:class:`~topsim.user.telescope.Telescope`). Pass the *class*,
        not an instance — the Simulation will instantiate it.
    planning_model : ~topsim.algorithms.planning.Planning
        An instance of a planning model (e.g. ``BatchPlanning('batch')``).
    scheduling : ~topsim.algorithms.scheduling.Scheduling
        An instance of a scheduling model (e.g. ``BatchProcessing()``).
    delay : ~topsim.core.delay.DelayModel, optional
        Optional delay model for task runtime variability.
    timestamp : float, optional
        Optional timestamp for reproducible output filenames.
    to_file : bool, optional
        If True, write simulation output to an HDF5 file. If False (default),
        return DataFrames from :py:meth:`start`.
    hdf5_path : str, optional
        Path for HDF5 output when ``to_file=True``.
    ...
    >>> env = simpy.Environment()
    >>> sim = Simulation(
    ...     env, 'path/to/config.json', Telescope,
    ...     BatchPlanning('batch'), BatchProcessing()
    ... )
    >>> global_df, task_df = sim.start()

    With delay modelling:

    >>> dm = DelayModel(0.1, 'normal', DelayModel.DelayDegree.LOW)
    >>> sim = Simulation(
    ...     env, config, Telescope, plan, sched, delay=dm
    ... )
    """
