...
class Config:
    ...
    The configuration is a JSON file with three top-level keys:
    ``instrument``, ``cluster``, and ``buffer``, plus an optional
    ``timestep`` key. This format is compatible with configurations
    generated by ``skaworkflows``.
    ...
    timestep_unit : str
        Granularity of simulation time (``'seconds'``, ``'minutes'``,
        ``'hours'``, or an integer for a custom unit).

    Raises
    ------
    OSError
        Raised if the configuration file cannot be read or does not exist.
    json.JSONDecodeError
        Raised if the JSON file is malformed.
    KeyError
        Raised if a required top-level key is missing from the JSON.
    """
    ...
    def parse_cluster_config(self):
        """
        Parse the cluster section of the simulation configuration.

        Reads machine specifications from ``self.cluster['system']['resources']``
        and applies the timestep multiplier to CPU (flops), compute bandwidth,
        and ethernet bandwidth values. Supports both the current
        ``count``-based format and the deprecated per-machine format.

        Returns
        -------
        tuple
            (machine_list, system_bandwidth) where machine_list is a list of
            :py:class:`~topsim.core.machine.Machine` objects and
            system_bandwidth is the cluster-wide ethernet bandwidth.
        """
        try:
    ...
    def parse_instrument_config(self, instrument_name):
        """
        Parse the instrument section for a given instrument name.

        Reads the observation schedule, pipeline definitions, and instrument
        capacity from the configuration. Converts observation start times and
        durations based on the configured timestep unit.

        Parameters
        ----------
        instrument_name : str
            Name of the instrument (e.g. ``'telescope'``).

        Returns
        -------
        tuple
            (total_arrays, pipelines, observations, max_ingest_resources)
        """
        timestep_multiplier = 1
        if self.timestep_unit == 'minutes':
            timestep_multiplier = 60
    ...
    def parse_buffer_config(self):
        """
        Parse the buffer section of the simulation configuration.

        Returns
        -------
        tuple
            (hot_buffers, cold_buffers) where each is a dict mapping
            buffer IDs to :py:class:`~topsim.core.buffer.HotBuffer` and
            :py:class:`~topsim.core.buffer.ColdBuffer` instances.
        """
        config = self.buffer
        timestep_multiplier = 1
        if self.timestep_unit == 'minutes':
    ...
    def get_max_ingest(self, instrument_name):
        """
        Return the maximum number of ingest resources for the given instrument.

        Parameters
        ----------
        instrument_name : str
            Instrument name as it appears in the config.

        Returns
        -------
        int
            Maximum number of resources that can be used for ingest pipelines.
        """
