...
class DynamicSchedulingFromPlan(Scheduling):
    """
    Scheduling algorithm that follows a pre-computed static plan.

    At each timestep, this algorithm provisions resources for an
    observation (if not already provisioned), then allocates ready tasks
    from the workflow plan to the machines specified in the static
    schedule. Tasks whose predecessors are not yet complete are skipped
    until they become ready.

    Supports HEFT-style static plans generated by
